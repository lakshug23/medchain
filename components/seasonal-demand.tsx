...
interface SeasonalDemandProps {
  selectedMedicine?: string
  selectedRegion?: string
}

// Generate seasonal data based on selected medicine and region
const generateSeasonalData = (medicine?: string, region?: string) => {
  let baseData = [
...
export function SeasonalDemand({ selectedMedicine, selectedRegion }: SeasonalDemandProps) {
    ...
    if (selectedMedicine && selectedRegion) {
      ...
      fetch("/api/seasonal", {
        ...
        .catch((err) => {
          ...
          setSeasonalData(generateSeasonalData(selectedMedicine, selectedRegion))
          setLoading(false)
        })
    }
  ...
  const title =
    selectedMedicine && selectedRegion ? `Seasonal Demand Pattern - ${selectedMedicine}` : "Seasonal Demand Pattern"
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {seasonalData.map((season, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-4 rounded-lg border border-white/20 bg-white/5"
                    >
